# -*- coding: utf-8 -*-
"""
Created on Wed Jul 27 16:55:39 2022
...
def get_relative_att_stack (trans_imgs_dict, dst_dir, overwrite = False, HE_LE = ([10,20],[75,85]), proc_folder = []):
    ...
    for key, value in trans_imgs_dict.items():
        ...
        for imgs_acq in value:
            ...
            HE_img = np.log(np.nanmean(list_imgs[HE_LE[0][0]:HE_LE[0][1]], axis = 0))
            LE_img = np.log(np.nanmean(list_imgs[HE_LE[1][0]:HE_LE[1][1]], axis = 0))
            
            rel_att_hdr = list_hdrs[int((HE_LE[0][0]+HE_LE[0][1])/2)]
            
            att_rel_img = (np.nan_to_num(LE_img/HE_img), rel_att_hdr)
            
            rel_att_hdr = update_header (att_rel_img[1], update_type = 'HISTORY', update_list = ['Relative attenuation, range HE: ' + str(HE_LE[0]) + ' and LE: ' + str(HE_LE[1])])
            
            list_imgs_rel_att.append(att_rel_img)
            
            
        for n, img_rel_att in zip(tqdm(range(len(value)), desc = 'Writing Relative Attenuation Images'), list_imgs_rel_att):
            ...
            write_img(img_rel_att, file_name, base_dir='', overwrite = overwrite)
