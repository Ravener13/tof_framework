...
def mcpoc_read_summedimg(src_dir, exp_name):
    import time
        # compute the file name
    fname = os.path.join(src_dir, exp_name + '_SummedImg.fits')
...
def mcpoc_write_img(dst_img, dst_dir, dst_name, index, timestamp):
    ...
    hdu.header ['COMMENT'] = 'User: ' + os.getlogin()
    hdu.header ['COMMENT'] = 'Date Processed (Overlap Correction): ' + str(datetime.datetime.now())
    hdu.writeto(fname)
...
def mcpoc_correct_exp(src_dir, dst_dir, exp_name, ref_sh_count, binning, info_dataframe_dir, scale=1.0,
                      savetype=float, this_index = [0,1000]):
    ...
    if ref_sh_count == 'auto':
        ref_sh_count = np.mean(sh_counts)
    
    if ref_sh_count == 'IMAT auto':
        new_exp_name, ref_sh_count = IMAT_correction (exp_name, info_dataframe_dir, frequency = 5)
    ...
    for i_sh in range(n_shut):
        ...
        for i_bin in range(imin, imax+1):
            ...
            bin_list.append(dst_img)
            
            if len(bin_list) == binning:
                
                img_bin = np.mean(bin_list,axis = 0)
                    # get the source image for between specific indexes
                if idx_count >= this_index[0] and idx_count <= this_index[1]:
                    mcpoc_write_img(img_bin, dst_dir, exp_name, idx_count, timestamp)
                idx_count+=1
                bin_list = []
...
def mcpoc_correct_batch(src_dir, dst_basedir, ref_sh_count = 'auto', exp_threshold= 0 , binning = 1, scale=1.0, this_index = [0,1000],
                        savetype=float, desc='', info_dataframe_dir = ''):
    import warnings
    ...
    if info_dataframe_dir != '':
        info_dataframe_dir = pd.read_csv (info_dataframe_dir, sep = '\t')
        
            # filter the list, in this specific case for measurements above 6.667 min and below 4.6667
        filtered_times = info_dataframe_dir.loc[(info_dataframe_dir['Time sec'] >= 400) | (info_dataframe_dir['Time sec'] <= 280)]
        
            # afetr filter, take just the names
        filtered_names = filtered_times['Name'].tolist()
        print('Experiments that are above 6:40 min or below 4:20 min are: ')
    ...
    for exp_dir in glob.glob(src_dir + '/*/'):
        ...
        for i in tqdm(range(len(exp_names)), desc=desc):
            ...
            summedimg,_ = mcpoc_read_summedimg(exp_dir, exp_name)
                
            if np.mean(summedimg) > exp_threshold :
                
                    # if you are not working with info_data_frame dir, comment the line blow and change the "new_exp_name" to exp_name"
                new_exp_name, ref_sh_count = IMAT_correction (exp_name, info_dataframe_dir, frequency = 5)
                    # compute the folder name of the destination stack
                dst_dir = os.path.join(dst_basedir,os.path.basename(normpath(exp_dir)), 'Corrected_' + new_exp_name +'_set_' +
                                       format(i, '03d'))
                ...
                mcpoc_correct_exp(exp_dir, dst_dir, exp_name, ref_sh_count, binning = binning, info_dataframe_dir = info_dataframe_dir,
                                  scale=scale, savetype=savetype, this_index = this_index)
            else:
                list_not_processed.append(exp_name)
    print('Experiments not processed due to low counts (counts threshold) are: ', list_not_processed)
...
def txt_timestamps (src_dir, dst_dir):
    ...
    This function is meant to be used with 'the overlap_corr_mod' and will serve as an evaluation of the 
        OB weightening.
...
def IMAT_correction (exp_name, info_exp, frequency = 5):
    
    tags = exp_name.split('_')
    
    run_no = int(tags[0][4:]) + int(tags[-1])
    
    name = info_exp[info_exp['Run No.'] == run_no]['Name'].values[0]
    time_sec = info_exp[info_exp['Run No.'] == run_no]['Time sec'].values[0]
    
    tags[0] = name
    exp_name = '_'.join(tags)
    
    ref_sh_count = time_sec*frequency #because we used 5Hz so it is 300 per minute
    
    return exp_name, ref_sh_count
...
# def IMAT_filter_time (exp_name, info_exp, frequency = 5, real_time_sec = 10000, time_threshold = 90):
...
#     max_time = real_time_sec + time_theshold
#     min_time = real_time_sec - time_theshold
    
#     info_exp[info_exp['Time sec']>=500]['Name']
    
#     test_time = 
    
#     return exp_name, time_sec