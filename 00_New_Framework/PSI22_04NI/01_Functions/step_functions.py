...
def pre_processing_step (stack_dict, pre_proc_seq, param_dict, **kwargs):
    ...
    for idx in range (len(seq)):
        ...
        if 'stack_averaging' in func_names:
            ...
            proc_dict = exec_bin_acquisitions_dict(proc_dict, sequence = seq[idx] , **proc_param)
        else:
...
def saving_step (stack_dict, dst_dir, img_name = 'test', save_energies = False, **kwargs):
    ...
    save_proc_dict = copy(stack_dict)
...
def full_processing (src_dir, dst_dir, proc_folder, sequence, proc_parameters, img_name = 'transmission', save = True, testing = False, **kwargs):
    ...
    weights_DF = weighting_func (src_dir)
    param_dict = copy(proc_parameters)
    ...
    functions_dict = {'scrubbing_correction_dict' : scrubbing_correction_dict,
                      ...
                      'slice_values_dict' : slice_values_dict,
                      'intensity_correction_dict' : intensity_correction_dict,
                      'referencing_dict' : referencing_dict,
                      'image_binning_dict' : image_binning_dict
                      }
    
    for folder in proc_folder:
        
        res_dict = copy(proc_dict)
        print('Experiment ' + folder + ' in process...')
        
        keep_folder = [folder]
        keep_key_weights (res_dict, weights_DF, keep_folder)
        
        for function in sequence:
            
            if function.__name__ not in functions_dict.keys():
                
                res_dict = pre_processing_step (res_dict, pre_proc_seq = [function], param_dict = param_dict, **kwargs)
                
            else:
                res_dict = processing_step (res_dict, proc_seq = [function], param_dict = param_dict, **kwargs)
                
        if save:
            saving_step (res_dict, dst_dir, img_name = img_name, **param_dict)
