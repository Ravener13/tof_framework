...
#                         slice_acq_set
# =============================================================================
def slice_acq_set (acq_set, start_slice = 0, end_slice = '', **kwargs):
    '''
    ...
    if end_slice  == '':
        end_slice = len(acq_set)
        
    return acq_set[start_slice:end_slice]
...
def img_align(src_img, ref_img, reg_rois_list, max_shift=10, subpix=10, dof=['tx','ty','sx','sy'], debug_data={}, **kwargs):
    ...
    for i, roi_def in enumerate(reg_rois_list):
...
def img_registration (src_img, ref, reg_rois_list ='', dof=['tx','ty','sx','sy'], M = '', **kwargs):
    '''
    ...
    hdr = src_img[1]
    if M == '':
        M = img_align(img, ref[0], reg_rois_list =reg_rois_list, max_shift=15, subpix=15, dof=dof, debug_data={})
...
#                           TFC_corr
# =============================================================================
def TFC_corr(src_img, ref_img_TFC, TFC, use_ref = True, **kwargs):

    img = src_img[0].copy()
    hdr = src_img[1]
    ref = ref_img_TFC[0].copy()
    
    if use_ref:
        img_corr = (img/ref)*TFC
        hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: TFC correction. TFC: ' + str(TFC)])
    
    else:
        img_corr = img*TFC
        hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: TFC correction without dividing by reference. TFC: ' + str(TFC)])
...
#                           referencing_corr
# =============================================================================
def referencing_corr(src_img, referencing_img, **kwargs):

    img = src_img[0].copy()
    hdr = src_img[1]
    ref = referencing_img[0].copy()
    
    img_corr = img/ref
    hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: Referencing correction.'])
...
#                           intensity_corr
# =============================================================================
def intensity_corr(src_img, int_corr, **kwargs):

    img = src_img[0].copy()
    hdr = src_img[1]
    
    img_corr = img*int_corr
    hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: Intensity correction. Factor: ' + str(int_corr)])
...
#                           relative_att_img
# =============================================================================
def relative_att_img(img_HE, img_LE, HE_range = [28,43], LE_range = [83,94], **kwargs):

    import warnings
    warnings.filterwarnings("ignore")
    
    
    HE_img = np.log(img_HE[0])
    LE_img = np.log(img_LE[0])

    hdr = img_HE[1]
    
    att_rel_img = np.nan_to_num(LE_img/HE_img)
    
    hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: Relative attenuation. Range HE: ' + str(HE_range) + ', LE: ' + str(LE_range)])
    
    return (att_rel_img, hdr)
...
def BB_intensity_corr(src_img, BB_mask=None, avg_val=None, **kwargs):
    '''
    corrects the image for black body (BB) transmission by subtracting the 
    average transmission in the BB area from the entire image array
    the BB average can either be calculated as average over the whole
    acquisition or over a single frame

    Parameters
    ----------
    src_img : tuple
        contains the image np array to be altered and its image header dict
    BB_mask : np mask array, optional
        mask marking the BB position
    avg_val: float, optional
        average BB value to substract in case over averaged over acquisition
    **kwargs : dict
    ...
    -------
    img_corr : np array
        2D array corresponding to the BB intensity corrected image
    hdr : dict
        header: metadata of image with updated image processing steps

    TODO
    ----
    maybe replace negative values by 0
    '''

    img = src_img[0].copy()
    hdr = src_img[1]

    # check if BB average still has to be calculated
    if avg_val is None:
        BB_img = src_img[0].copy()
        avg_val = BB_get_avg(BB_img, BB_mask[0])

    # substract calculated/given avg_val from image
    img_corr = img - avg_val
    hdr = update_header(hdr, update_type='HISTORY', update_list=[
                        'Process: BB intensity correction. Subtracted: ' + str(avg_val)])
...
def BB_get_avg(src_img, BB_mask, **kwargs):
    '''
    returns the average value of the BB pixels in a given image
    used e.g. for substracting BB noise from whole image -> BB_intensity_corr

    Parameters
    ----------
    src_img : np array
        2D array with pixel values of img
    BB_mask : np array
        same dimension as src_img indicating BB pixels with value=255 and all 
        other pixels represented by 0
    **kwargs : dict
        collection of additional named parameters
    ...
    -------
    avg : float
        average pixel value of the BB pixels in the image

    '''

    # replace all values not in the BB region with nan values
    src_img[BB_mask == 0] = np.nan

    # calculate average of BB pixel values with nanmean
    avg = np.nanmean(src_img)
...
def show_img_rois(img, title='', wmax=750, dr=[], stretch=[1.0,1.0], keep_fig=False,
    ...
    for rects, col in dr:
        ...
        for i, rect_def in enumerate(rlist):
            ...
            if show_num:
                sp.text(rx+rw/2, ry+rh/2, name, color='black', fontsize='medium',
                        fontweight= 'normal', ha='center', va='center', backgroundcolor=col)
...
#                              select_rois
# =============================================================================
def select_rois(src_img, list_rois = [], title = 'Select ROI' , flip_img = False, **kwargs):
    '''
    This function provides an interactive interface for selecting a rectangular region of interest in an image.
    This function is meant to be used with the `%load`  magic in jupyter.
