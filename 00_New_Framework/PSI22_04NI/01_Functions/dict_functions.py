...
def exec_bin_acquisitions_dict(stack_dict, sequence = [binning_acquisitions], count_time = False, acquisitions_binning_factor = 2 , **kwargs):
    ...
    **kwargs : extra arguments. dict
    ...
    for key,values in proc_dict.items():
        ...
        if len(values) < acquisitions_binning_factor:
            print ('Not enough acquisitions for binning, saving ' + key + ' as a single acquisition')
            seq_values =  values
            
        elif len(values) % acquisitions_binning_factor != 0:
            print ('Extra (final) acquisition left out due to binning factor')
            seq_values = binning_acquisitions (values[:-1], acquisitions_binning_factor = acquisitions_binning_factor, **kwargs)
...
def slice_values_dict (stack_dict, start_slice = '', end_slice = '', **kwargs):
    ...
    for key, value in proc_dict.items():
        ...
        for idx, imgs_acq, hdrs_acq in zip (tqdm(range(len(value)), desc = 'Slicing dictionary'), list_imgs, list_hdrs):
            ...
            if end_slice == '':
                end_slice = len(imgs_acq)
                
            slice_acq = imgs_acq[start_slice:end_slice]
            slice_hdr = hdrs_acq[start_slice:end_slice]
            
            for img, hdr in zip(slice_acq, slice_hdr):
                
                hdr_final = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: slice acquisition. Slice range : ' + str(start_slice) + ' - ' + str(end_slice)])
...
def image_binning_dict (stack_dict, new_shape = (256,256), **kwargs):
    
    def binning_2d(arr, new_shape):
    ...
    for key, value in proc_dict.items():
        ...
        for idx, imgs_acq, hdrs_acq in zip (tqdm(range(len(value)), desc = 'Binning arrays in dictionary'),list_imgs, list_hdrs):
            
            binned = []
            
            for img, hdr in zip (imgs_acq,hdrs_acq):
                
                hdr_final = update_header (hdr, update_type = 'HISTORY', update_list = ['Array binning. New shape: ' + str(new_shape)])
                new_img = (binning_2d(img, new_shape),hdr)
                binned.append(new_img)
            
            folder_values.append(binned)
