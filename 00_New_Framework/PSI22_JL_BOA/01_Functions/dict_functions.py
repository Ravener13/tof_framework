...
def BB_intensity_correction_dict(stack_dict, BB_mask, BB_avg=False, **kwargs):
    '''
    ...
    ----------
    stack_dict : dictionary
        folder structured dict with references to the image numpy arrays and 
        image headers
    mask : tuple

    acq_qvg : bool, optional
        to decide wheter the quotient factor is calculated as average over the
        whole acqusition or per frame (default). Default is False.
    **kwargs : dict
    ...
    -------
    new_dict : dictionary
        same structure as stack_dict but with updated images for which the 
        average BB pixel value was subtracted
    ...
    for key, value in proc_dict.items():
        ...
        for idx, imgs_acq in zip(tqdm(range(len(value)), desc='Processing BB Intensity Correction'), value):

            intensity_corr_vals = []

            # stays none if BB mean is calculated per frame
            avg_val = None

            # if averaging BB value over whole acquisition
            if BB_avg:

                # list for BB average values
                BB_avgs = []

                # loop over frames in acquisition and calculate BB mean per frame
                for img in imgs_acq:
                    BB_avgs.append(BB_get_avg(img[0], BB_mask))

                # average BB means over whole acquisition
                avg_val = np.nanmean(BB_avgs)

            for img in imgs_acq:
                corr_img = BB_intensity_corr(
                    img, BB_mask=BB_mask, avg_val=avg_val)
                intensity_corr_vals.append(corr_img)
