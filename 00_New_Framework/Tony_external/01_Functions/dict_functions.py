...
def exec_filters_dict(stack_dict, sequence = '', count_time = False, **kwargs):
    ...
    **kwargs : extra arguments. dict
    ...
    for key,values in proc_dict.items():
        ...
        for idx, imgs_acq in zip (tqdm(range(len(values)), desc = 'Processing Filtering '),values):
...
def TFC_correction_dict (stack_dict, ref_dict= '', ref_img_TFC = '', nca = [0,0,0,0], use_ref = True, **Kwargs):
    ...
    if ref_dict == '' and ref_img_TFC == '':
        print('Using proc_dict as both img and ref to calculate TFC')
        ref_dict = deepcopy (stack_dict)
        
    if ref_dict != '' and ref_img_TFC == '':
        
        ref_imgs = [ref_val for ref_val in ref_dict.values()][0][0]
        ref_imgs_arr = [ref_arr[0] for ref_arr in ref_imgs]
        ref_imgs_hdr = [ref_arr[1] for ref_arr in ref_imgs]
        ref_img_TFC = (np.nanmean(ref_imgs_arr, axis = 0), ref_imgs_hdr[int(len(ref_imgs_hdr)/2)])
                       
    else:
        return print('Missing reference TFC image or ref_dict')
    ...
    for key, value in proc_dict.items():
        ...
        for idx, imgs_acq, TFC_img in zip (tqdm(range(len(value)), desc = 'Processing TFC Correction'), value, list_imgs_TFC):
            ...
            for img, ref in zip(imgs_acq, ref_imgs):
                corr_img = TFC_corr(img, ref, TFC , use_ref = use_ref)
    ...
    for key, value in stack_dict.items():
        ...
        for acq_numb, imgs_acq in zip (tqdm(range(len(value)), desc = 'Writing Images'),value):
            ...
            for img in imgs_acq:
                ...
                file_name = os.path.join(dst_dir, folder_name, acq_folder, key + '_' + img_name + '_' + format(idx, '05d') + '.fits')
...
def save_energies_dict (stack_dict, dst_dir, folder_name = '', img_name = 'transmission', start_img_numb = 0, overwrite = False, HE_LE = (False, [15,30],[30,50]), **kwargs):
    ...
    for key, value in stack_dict.items():
        ...
        for n, img_HE, img_LE in zip(tqdm(range(len(value)), desc = 'Writing HE and LE Images'), list_imgs_HE, list_imgs_LE):
            ...
            file_name_HE = os.path.join(dst_dir, folder_name, 'HE_stack', key + '_' + img_name + '_' + format(idx, '05d') + '.fits')
            file_name_LE = os.path.join(dst_dir, folder_name, 'LE_stack', key + '_' + img_name + '_' + format(idx, '05d') + '.fits')
...
def slice_values_dict (stack_dict, start_slice = 0, end_slice = '', **kwargs):
    ...
    for key, value in proc_dict.items():
        ...
        for idx, imgs_acq, hdrs_acq in zip (tqdm(range(len(value)), desc = 'Slicing dictionary'), list_imgs, list_hdrs):
            ...
            for img, hdr in zip(slice_acq, slice_dhr):
                slice_set.append(img, hdr)
...
def extract_img_dict (stack_dict, key, acq_number = 0, img_number =0, **kwargs):
    img = deepcopy(stack_dict[key][acq_number][img_number])
    return img
