...
# =============================================================================
 #                         translate_img
# =============================================================================
def translate_img(src_img, translate, **kwargs):
    # translate is a 2d array  tranlate = [0,2] -> [vertical,horizontal]
    img = src_img[0].copy()
    hdr = src_img[1]
    
    img_corr = np.roll(img, translate[0], axis=0) # shift x place in vertilca axis
    img_corr = np.roll(img_corr, translate[1], axis=1) # shift y place in horizontal axis
...
#                           relative_att_img
# =============================================================================
def relative_att_img(img_HE, img_LE, HE_range = [28,43], LE_range = [83,94], casing_corr = [1,1], **kwargs):

    import warnings
    warnings.filterwarnings("ignore")
    
    cas_corr_HE = casing_corr[0]
    cas_corr_LE = casing_corr[1]
    ...
    if cas_corr_HE != 1 and cas_corr_LE != 1:
        
        img_HE = (img_HE/cas_corr_HE)
        img_LE = (img_LE/cas_corr_LE)
        
        hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: Removed casing. Conribution HE: ' + str(cas_corr_HE) + ', LE: ' + str(cas_corr_LE)])
        
    HE_img = np.log(img_HE)
    LE_img = np.log(img_LE)
...
#                         ob_roi_mod
# =============================================================================
#this function was made to correct a black line in the image 
#It is a mirror od the same BB area but from the left side

def mod_for_ob (src_img, roi_original=[0,0,50,512], roi_mod=[462,0,50,512]):
    
    img = src_img[0].copy()
    hdr = src_img[1]
    
        #for this you can use np.flip(roi_img,1) or np.fliplr(roi_img)
    roi_orig_mod = np.fliplr(crop_img(src_img, roi_original)[0])
    
    img[roi_mod[1]:roi_mod[1]+roi_mod[3],roi_mod[0]:roi_mod[0]+roi_mod[2]] = roi_orig_mod
    
    hdr = update_header (hdr, update_type = 'HISTORY', update_list = ['Process: ob corr mod'])
