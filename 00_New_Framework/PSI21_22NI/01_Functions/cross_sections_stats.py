...
def get_roi_values (dictionary, rois_dict, spectra_file, save_path ='', binning = 1, flight_path = 1, start_slice = '', end_slice='', name_xlsx = 'Transmission_values.xlsx', save_results = False, **kwargs):
    ...
    for roi_name, rois in rois_dict.items():
        ...
        for k, folder_img in enumerate(subfolder):
            ...
            for ndx, img in enumerate (folder_img):
                ...
                for idx, roi_val in enumerate(rois):
                    ...
                    results_vec.append(np.nanmean(crop_img(img, roi_val)[0]))
            
            #data =  pd.DataFrame({'Values_acq_' + str(k):results_vec})
            #all_imgs.append(results_vec)
            #for k in range(results_vec.shape[1]):
            Values['Values_acq_' + str(k)] = results_vec
    ...
    if end_slice == '':        end_slice= len(table_wvl)
...
def get_df_list_rois (list_imgs, list_rois, save_path ='', name_xlsx = 'Transmission_values.xlsx', save_results = False, **kwargs):
    
    import numpy as np
    import pandas as pd
    
    Values = pd.DataFrame(pd.np.empty((len(list_imgs), len(list_rois))))
    
    for i, roi in enumerate(list_rois):
        
        for idx, img in enumerate(list_imgs):
        
            Values.at[idx, i] = np.nanmean(crop_img(img, roi)[0])
        
    Values.index = ['Image_' + format(i, '03d') for i in range (len(list_imgs)) ]
    Values.columns = ['ROI_' + format(n, '02d') for n in range (len(list_rois)) ]
    
        # Send to the results to the directory from which you extracted the transmission images
    if save_results:
        Values.to_excel(save_path + '/' + name_xlsx)
...
def get_relative_att_stack (stack_dict, dst_dir, HE_n_LE, proc_folder = [], overwrite = False, save = True, casing_corr = [1,1], **kwargs):
    
    import warnings
    warnings.filterwarnings("ignore")
    
    trans_imgs_dict = copy(stack_dict)
    ...
    for key,values in trans_imgs_dict.items():
        ...
        HE_avg = avg_acquisition_dict (values, output_type = 'img', start_slice = HE[0], end_slice = HE[1])
        LE_avg = avg_acquisition_dict (values, output_type = 'img', start_slice = LE[0], end_slice = LE[1])
        
        rel_img =relative_att_img(HE_avg, LE_avg, HE_range = HE, LE_range = LE, casing_corr = casing_corr)
        
        if save:
            file_name = os.path.join(dst_dir, key + '_rel_att.fits' )
            write_img(rel_img, file_name, base_dir='', overwrite = overwrite)
            res_stack.append(rel_img)
...
def get_cross_sections (transmission_values_tab, compounds_dict, requested_cs , dst_dir = '' , save_table = True, name_xlsx = 'CS_results.xlsx', **kwargs):
...
def divide_DF (num_df, den_df, name_wvl = 'Wavelength [Å]'):
    ...
    DF_div = num_df_raw.iloc[:,:].div(den_df_raw.iloc[:,0], axis=0)
