...
# =============================================================================
#                        stack_batch_proc...... NO REFERENCE IMAGE
# =============================================================================
def stack_batch_proc (src_dir, dst_dir, img_mask_img, #ref, 
                      proc_folder = [], avg_seq = [stack_avg], save_results = True, final_HE_LE = (False, [3,23],[60,80]), save_sbkg = False, **kwargs):
    '''
...
# =============================================================================
#                      outlier_removal_range
# =============================================================================

def outlier_removal_range (img, threshold = [0,1.5], **kwargs):
    mask_x = np.where(img <= threshold[0])[0]
    mask_y = np.where(img <= threshold[0])[1]
    
    mask_x = np.append(mask_x, np.where(img >= threshold[1])[0])
    mask_y = np.append(mask_y, np.where(img >= threshold[1])[1])
    
    for x, y in zip(mask_x, mask_y) :
        slice = img[max(0, x-1):x+1, max(0,y-1):y+1] # assuming you want 5x5 square
        img[x,y] = np.mean([i for i in slice.flatten() if i > threshold[0] or i < threshold[1]])  # threshold is 0
    return img



    # does the fast transmission image nd find future outliers. Then, it takes those coordinates and, from the real images, corrects those pixels
def find_future_outlier (img, ref, trans_threshold = [0,1.5], trans_roi = [50, 4, 413, 503]):
        #do the transmission image
    trans_img =img/ref
    
        # create the mask for the values thta will contain an outlier
    mask_x = np.where(trans_img <= trans_threshold[0])[0]
    mask_y = np.where(trans_img <= trans_threshold[0])[1]
    
    mask_x = np.append(mask_x, np.where(trans_img >= trans_threshold[1])[0])
    mask_y = np.append(mask_y, np.where(trans_img >= trans_threshold[1])[1])
    
        # in the case the future transmission imahge will contain an outlier, remove them from the original images
    if len(mask_x) != 0:
        pre_thres_img = np.min(img [mask_x, mask_y])
        pre_thres_ref = np.min(ref [mask_x, mask_y])
        
        if pre_thres_img <=pre_thres_ref:
            thres_new = pre_thres_img
            
        else:
            thres_new = pre_thres_ref
                
        img_new = outlier_removal_range (img, threshold = [0,thres_new])
        ref_new = outlier_removal_range (ref, threshold = [0,thres_new])
        
    else:
        img_new = img
        ref_new = ref
        
    return img_new, ref_new
