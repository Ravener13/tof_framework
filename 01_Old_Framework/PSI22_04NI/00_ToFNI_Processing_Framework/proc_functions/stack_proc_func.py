...
#                        stack_batch_proc
# =============================================================================
def stack_batch_proc (src_dir, dst_dir, img_mask_img, proc_folder = [], avg_seq = '', save_results = True, save_sbkg = False, initial_HE_LE = (False, [3,23],[60,80]), final_HE_LE = (False, [3,23],[60,80]), **kwargs):
    '''
    ...
    #stack_dict = convert_stack_energies (stack_dict, HE_slice = [1, 21], LE_slice = [74,99])
    
        # computes the transmission images
    # stack_transmission_img (stack_dict, ref_mask_img, img_mask_img, ref_key = ref_folder, **kwargs)
    stack_transmission_img_no_ref(stack_dict, img_mask_img, save_sbkg = save_sbkg, **kwargs)
...
# =============================================================================
#                    stack_transmission_img_nore_ref
# =============================================================================
def stack_transmission_img_no_ref (dictionary, img_mask_img, print_TFC = False, save_sbkg = False, **kwargs):
    '''
    Computes the transmission images for the keys contained in a dictionary, with respect to a reference  (dry image)
    functional kwargs:
        nca = [x,y,w,h]. a list of int values to extrat the non-changing area, the default is [0,0,0,0]
    ...
    **kwargs : TYPE
    ...
    for key,value in dictionary.items():
        ...
        for i, src_img, in zip (np.arange(0, len(value)),value):
            
            #src_img_new , ref_img_new = find_future_outlier (src_img, ref_img, trans_threshold = [0,1.5], trans_roi = [50, 4, 413, 503])
                # intensity correction 
            test_img = np.array(np.ones(src_img.shape))
            TFC = TFC_corr(src_img, test_img, **kwargs)
            if  print_TFC:
                print(TFC)
            value [i] = (src_img/1.0)*TFC
