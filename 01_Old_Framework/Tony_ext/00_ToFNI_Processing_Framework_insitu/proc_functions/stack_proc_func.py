...
# =============================================================================
#                    stack_transmission_img FOR SA PROCESSING NO REF AND WITH TFC
# =============================================================================
def stack_transmission_img (dictionary, ref_mask_img, img_mask_img, ref_key = '', print_TFC = False, save_sbkg = False, **kwargs):
    ...
    **kwargs : TYPE
    ...
    for key_up,value_up in dictionary.items():
        print (key_up)
            # this takes care if the BB mask for the ref image is different as for the stacks
        if key_up == ref_key:
            for key, value in value_up.items():
                
                list_corr_sbkg = []
                for i, ref in zip (tqdm(range(len(value)), desc = 'Processing'),value):
                    sbkg_ref = SBKG_Wmask (ref, ref_mask_img, **kwargs)
                    list_corr_sbkg.append((ref[0] - sbkg_ref[0], ref[1]))
                # print('finished and changing values in '+ key)
                dictionary[key_up][key] = list_corr_sbkg
            ...
            for key, value in value_up.items():
                ...
                for i, img in zip (tqdm(range(len(value)), desc = 'Processing'),value):
                    ...
                    list_corr_sbkg.append((img[0] - sbkg_img[0], img[1]))
                dictionary[key_up][key] = list_corr_sbkg
    ...
    for key_up,value_up in dictionary.items():
        keys = [item for item in value_up.keys()]
        n=0
        for value_img, value_ref in zip (value_up.values(), ref_dict.values()):
        
            list_img = [elem[0] for elem in value_img]
            list_ref = [elem[0] for elem in value_ref]
            
            img = np.nanmean(list_img, axis = 0)
            ref = np.nanmean(list_ref, axis = 0)
            
            TFC = TFC_corr(img, ref, **kwargs)
            print(TFC)
            list_corr_img = []
            list_corr_ref = []
            list_corr_trans = []
            for i, src_img, ref_img in zip (np.arange(0, len(value_img)), value_img, value_ref):
                ...
                header_img = src_img[1]
                header_img['TFC'] = TFC
                
                header_ref = ref_img[1]
                header_ref['TFC'] = TFC
                
                list_corr_img.append((src_img[0]*TFC, header_img))
                list_corr_ref.append((ref_img[0]*TFC, header_ref))
                list_corr_trans.append(((src_img[0]/ref_img[0])*TFC, header_img))
            
            dictionary[key_up][keys[n]] = list_corr_img
            ref_dict[keys[n]] = list_corr_ref
            trans_dict[keys[n]] = list_corr_trans
            n+=1
            
            #merge both dictionaries
    dict_ref['00_ref_so'] = ref_dict
    dict_trans['01_trans_sa_so'] = trans_dict
    dictionary.update(dict_ref)
    dictionary.update(dict_trans)
...
#             list_HE = [elem[0][0] for elem in value]
#             list_LE = [elem[1][0] for elem in value]
                
#             img_ref_HE = np.nanmean(list_HE, axis = 0)
#             img_ref_LE = np.nanmean(list_LE, axis = 0)

#             ref_values = [img_ref_HE, img_ref_LE]
#             ref_corr = []
#             for i, ref in zip (tqdm(range(len(ref_values)), desc = 'Processing'),ref_values):
#                 sbkg_ref = SBKG_Wmask (ref, ref_mask_img, **kwargs)
#                 ref_corr.append(ref - sbkg_ref[0])
                
#                 # fits.writeto('J:/900 Varia/2021/000_tony_data/03_Processed_step_by_step/sbkg_ref/' + 'SBKG_ref' + str(n).zfill(5) +  '.fits', sbkg_ref[0])
#                 # fits.writeto('J:/900 Varia/2021/000_tony_data/03_Processed_step_by_step/sbkg_ref_min_sbkg/' + 'ref_min_sbkg_' + str(n).zfill(5) +  '.fits', ref - sbkg_ref[0])
#                 # n+=1
...
#             header_HE = [elem[0][1] for elem in value]
#             header_LE = [elem[1][1] for elem in value]
#             n=0
#             for i, img_HE, img_LE in zip (tqdm(range(len(value)), desc = 'Processing'),list_HE,list_LE):
#             #for i, img_HE, img_LE in zip (tqdm(range(len(value)), desc = 'Processing'),value):
#                 sbkg_img = SBKG_Wmask (img_HE, img_mask_img, **kwargs)
#                 img_HE = (img_HE - sbkg_img[0], header_HE[i])
                
#                 # fits.writeto('J:/900 Varia/2021/000_tony_data/03_Processed_step_by_step/sbkg_exp/' + 'SBKG_exp_HE_' + str(n).zfill(5) +  '.fits', sbkg_img[0])
#                 # fits.writeto('J:/900 Varia/2021/000_tony_data/03_Processed_step_by_step/sbkg_exp_min_exp/' + 'exp_min_sbkg_HE_' + str(n).zfill(5) +  '.fits', img_HE[0])
                
#                 sbkg_img = SBKG_Wmask (img_LE, img_mask_img, **kwargs)
#                 img_LE = (img_LE - sbkg_img[0], header_LE[i])
                
#                 # fits.writeto('J:/900 Varia/2021/000_tony_data/03_Processed_step_by_step/sbkg_exp/' + 'SBKG_exp_LE_' + str(n).zfill(5) +  '.fits', sbkg_img[0])
#                 # fits.writeto('J:/900 Varia/2021/000_tony_data/03_Processed_step_by_step/sbkg_exp_min_exp/' + 'exp_min_sbkg_LE_' + str(n).zfill(5) +  '.fits', img_LE[0])
#                 # n+=1
#                 value[i] = [img_HE, img_LE]
...
#         for i, img_HE, img_LE in zip (np.arange(0, len(value)),list_HE, list_LE):
#                 # intensity correction 
#             TFC_HE = TFC_corr(img_HE, ref_dict[0], **kwargs)
#             TFC_LE = TFC_corr(img_LE, ref_dict[1], **kwargs)
#             if  print_TFC:
#                 print(TFC_HE)
#                 print(TFC_LE)
#             header_HE[i]['TFC_HE'] = TFC_HE
#             header_LE[i]['TFC_LE'] = TFC_LE
#             img_HE = ((img_HE/ref_dict[0])*TFC_HE, header_HE[i])
#             img_LE = ((img_LE/ref_dict[1])*TFC_LE, header_LE[i])
#             value [i] = [img_HE, img_LE]
#     return 
    ...
    for key, values in dictionary.items():
        ...
        for img in values:
            ...
            if testing_mode:
                ...
                file_name = os.path.join(dst_dir, key, key + '_transmission_' + format(index, '05d') + '.fits')
                
                trans_img = (np.flip(img_arr, 0), img_header)
                    # utilize pyerre's function to save each image in the folder
                write_img(trans_img, file_name, base_dir='', overwrite=False)
...
#                        stack_batch_proc
# =============================================================================
def stack_batch_proc (src_dir, dst_dir, ref_mask_img, img_mask_img, reg_img, proc_folder = [], ref_folder = '15_ref_bb', avg_seq = '', save_results = True, scrubbing = True, initial_HE_LE = (False, [3,23],[60,80]), final_HE_LE = (False, [3,23],[60,80]), **kwargs):
    '''
    ...
    if initial_HE_LE[0]:
        ...
        bin_HE_n_LE (stack_dict, HE_slice = [initial_HE_LE[1][0], initial_HE_LE[1][1]], LE_slice = [initial_HE_LE[2][0], initial_HE_LE[2][1]])
    ...
    # after_scrubb = stack_dict.copy()
    # for key, value in after_scrubb.items():
    #     after_scrubb_dir = dst_dir + '/after_scrubb/' + key
    #     if not os.path.exists(after_scrubb_dir):
    #         os.makedirs(after_scrubb_dir)
        
    #     save_dict(value, after_scrubb_dir, overwrite = True, **kwargs)
    
    img_registration_dict (stack_dict, proc_folder, reg_img,  **kwargs)
    
    # after_img_reg = stack_dict.copy()
    # for key, value in after_img_reg.items():
    #     after_img_reg_dir = dst_dir + '/after_im_reg/' + key
    #     if not os.path.exists(after_img_reg_dir):
    #         os.makedirs(after_img_reg_dir)
        
    #     save_dict(value, after_img_reg_dir,  overwrite = True, **kwargs)
    
    #stack_dict = convert_stack_energies (stack_dict, HE_slice = [15, 30], LE_slice = [30,50])
    ...
    #     save_img_energies (stack_dict, new_dst_dir, **kwargs)
    if scrubbing == True and save_results == True:
        ...
        for key, value in stack_dict.items():
            ...
            if final_HE_LE[0] == True:
                
                save_stack_energies (value, new_dst_dir, HE_slice = [final_HE_LE[1][0], final_HE_LE[1][1]], LE_slice = [final_HE_LE[2][0], final_HE_LE[2][1]], **kwargs)
            else:
                save_dict(value, new_dst_dir, **kwargs)
    ...
    elif scrubbing == False and save_results == True and final_HE_LE[0] == False:
        save_dict(stack_dict, dst_dir, **kwargs)
...
#                        save_img_energies
# =============================================================================
def save_img_energies (stack_dict, dst_dir, start_numb = 0, **kwargs):
    ...
    for key, values in stack_dict.items():
        ...
        list_HE = [elem[0][0] for elem in values]
        list_LE = [elem[1][0] for elem in values]

        header_HE = [elem[0][1] for elem in values]
        header_LE = [elem[1][1] for elem in values]

            # takes each value in the subfolder (values
            
        for i, img_HE, img_LE in zip (np.arange(0, len(list_HE)),list_HE, list_LE):
                # creates the saving path 
            file_name_HE = os.path.join(dst_dir , 'HE_stack', key + '_transmission_img_HE_' + format(index, '05d') + '.fits')
            file_name_LE = os.path.join(dst_dir , 'LE_stack', key + '_transmission_img_LE_' + format(index, '05d') + '.fits')
        
            img_HE = (np.flip(img_HE, 0), header_HE[i])
            img_LE = (np.flip(img_LE, 0), header_LE[i])
...
# =============================================================================
#                        convert_stack_energies
# =============================================================================
def convert_stack_energies (stack_dict, HE_slice = [1, 21], LE_slice = [74,99], testing_mode = False, **kwargs):
    ...
    if testing_mode != True:
        ...
        new_dict = {}
            ...
            # takes every key and its values in the dictionary
        for key_high, values_high in stack_dict.items():
            im_key_list = []
            for key, values in values_high.items():
                ...
                img_HE = (HE_avg, header_HE)
                img_LE = (LE_avg, header_LE)
                
                im_key_list.append([img_HE, img_LE])
            new_dict [key_high] = im_key_list
...
def img_registration (img, ref, num_iter = '', method = '', rois_list ='', dof=['tx','ty','sx','sy'], M = '', **kwargs):
    '''
    ...
    if method == '3':
        if M == '':
            M = img_align(img, ref, rois_list, max_shift=15, subpix=15, dof=dof, debug_data={})
        corrected_img = sp.ndimage.affine_transform(img, M, order=1)
...
def img_registration_dict (stack_dict, proc_folder, ref= np.zeros([512,512]), rois_list ='', dof=['tx','ty','sx','sy'] , scrubbing = True, M = '',  **kwargs):
                ...
                for sub_key, folder in stack_dict [key].items():
                    ...
                    for img in folder:
                        if M == '':
                            ...
                            M = img_align(img[0], ref[0], rois_list = rois_list, max_shift=15, subpix=15, dof=dof, debug_data={}, **kwargs)
                        corrected_img = sp.ndimage.affine_transform(img[0], M, order=1)
                        new_list.append((corrected_img, img[1]))
                    stack_dict [key][sub_key] = new_list
            
            else:
                new_folder_list = []
                for idx, folder in enumerate(value):
                    ...
                    for img in folder:
                        ...
                        corrected_img = sp.ndimage.affine_transform(img[0], M, order=1)
                        new_list.append((corrected_img, img[1]))
                    new_folder_list.append(new_list)
                stack_dict [key] = new_folder_list
...
def bin_HE_n_LE (stack_dict, HE_slice = [5, 19], LE_slice = [36,50]):
    
    for key, values in stack_dict.items():

        for idx, folder in enumerate(values):
            
            imgs = [img[0] for img in folder]
            
            HE_avg = np.nanmean(imgs[HE_slice[0]:HE_slice[1]], axis = 0)
            LE_avg = np.nanmean(imgs[LE_slice[0]:LE_slice[1]], axis = 0)
        
            header_HE = folder[int((HE_slice[0]+HE_slice[1])/2)][1]
            header_LE = folder[int((LE_slice[0]+LE_slice[1])/2)][1]
            
            img_HE = (HE_avg, header_HE)
            img_LE = (LE_avg, header_LE)
        
            values[idx] = [img_HE, img_LE]
    return
