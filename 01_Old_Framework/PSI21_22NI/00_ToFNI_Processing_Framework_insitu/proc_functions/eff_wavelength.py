...
def trapezoid_weight (x_values, y_values, point, base_bot_percentage = 32, base_top_percentage = 8):
    ...
    # top_base = len(x_values[mid_min:mid_max])
    # bot_base = len(x_val_slice)
    # f_base_top = interp1d(x_values[mid_min:mid_max], y_values[mid_min:mid_max])
    # xnew = np.linspace (x_values[mid_min:mid_max][0], x_values[mid_min:mid_max][-1],num=4)
    # ynew = f_base_top(xnew)
    
    # plt.plot(x_values[mid_min:mid_max], y_values[mid_min:mid_max], 'o', xnew, ynew, '-')
    # plt.show()

    return np.array(x_val_slice), weighted_array
