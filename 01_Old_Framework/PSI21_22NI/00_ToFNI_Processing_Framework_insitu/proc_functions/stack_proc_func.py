...
#                        stack_batch_proc
# =============================================================================
def stack_batch_proc (src_dir, dst_dir, ref_mask_img, img_mask_img, #reg_img
                      proc_folder = [], ref_folder = '15_ref_bb', avg_seq = '', save_results = True, scrubbing = True,  final_HE_LE = (False, [3,23],[60,80]), **kwargs):
    '''
...
def img_align(src_img, ref_img, rois_list, max_shift=15, subpix=10, dof=['tx','ty','sx','sy'], debug_data={}, **kwargs):
...
def img_registration (img, ref, num_iter = '', method = '', rois_list ='', dof=['tx','ty','sx','sy'], **kwargs):
    ...
    if method == '3':
        M = img_align(img, ref, rois_list, max_shift=15, subpix=10, dof=dof, debug_data={})
        corrected_img = sp.ndimage.affine_transform(img, M, order=1)
...
def img_registration_dict (stack_dict, proc_folder, ref= np.zeros([512,512]), rois_list ='', dof=['tx','ty','sx','sy'] , scrubbing = True,  **kwargs):
        ...
        if key in proc_folder:
            ...
            if scrubbing:
                for sub_key, folder in stack_dict [key].items():
                    new_list = []
                    for img in folder:
                        M = img_align(img[0], ref[0], rois_list = rois_list, max_shift=15, subpix=10, dof=dof, debug_data={}, **kwargs)
