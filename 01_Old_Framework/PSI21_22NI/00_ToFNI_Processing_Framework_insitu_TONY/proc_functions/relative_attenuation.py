...
# =============================================================================
#                          get_relative_att_HE_LE
# =============================================================================
    
def get_relative_att_HE_LE (dst_dir, HE_dir, LE_dir, start_numb = 0):
    ...
    HE_list = [os.path.abspath(os.path.join(HE_dir, file)) for file in os.listdir(HE_dir)]
    LE_list = [os.path.abspath(os.path.join(LE_dir, file)) for file in os.listdir(LE_dir)]

    for  HE_img, LE_img in zip (HE_list, LE_list):
        
            # get the image for HE and LE
        img_HE = get_img(HE_img)
        img_LE = get_img(LE_img)
        
        header_HE = img_HE[1]
        header_LE = img_LE[1]
        
            # get the relative attenuation image for each point PIXEL WISE
        att_rel_img = np.nan_to_num(img_LE[0]/img_HE[0])
           
        file_name = os.path.join(dst_dir, 'rel_att_img_' + format(idx, '05d') + '.fits' )
            
            #pack the image with the average value of the timestamps
        att_rel_img = (np.flip(att_rel_img,0), header_HE)
