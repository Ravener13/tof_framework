...
# =============================================================================
def exec_bin_frames_dict(stack_dict, sequence = [binning_frames], count_time = False, frames_binning_factor = 10 , start_img = 0, **kwargs):
    '''
    Averages image stacks after the overlap correction
    A sequence is required, this sequence can contain any function for image enhancement (filtering, outlier removal, etc.).
    The function 'stack_avg' does the averaging, thus it is required to be in any position inide the sequence
    ...
    **kwargs : extra arguments. dict
    ...
    for key,values in proc_dict.items():
        ...
        for idx, imgs_acq in zip (tqdm(range(len(values)), desc = 'Processing Binning Frames'),values):
            ...
            seq_values.append(binning_frames (imgs_acq, start_img = start_img, frames_binning_factor = frames_binning_factor))
...
def exec_bin_acquisitions_dict(stack_dict, sequence = [binning_acquisitions], count_time = False, acquisitions_binning_factor = 2 , **kwargs):
    ...
    **kwargs : extra arguments. dict
    ...
    for key,values in proc_dict.items():
        ...
        seq_values = binning_acquisitions (values, acquisitions_binning_factor = acquisitions_binning_factor)
...
def TFC_correction_dict (stack_dict, ref_img_TFC, nca = [0,0,0,0], use_ref = True, **Kwargs):
    ...
    for key, value in proc_dict.items():
        ...
        folder_values = []
        list_imgs_TFC = avg_frames_dict (proc_dict[key], output_type = 'frames')
        
        for idx, imgs_acq, TFC_img in zip (tqdm(range(len(value)), desc = 'Processing TFC Correction'),value, list_imgs_TFC):
            
            TFC_values = []
            if sum(nca) == 0:
                TFC = 1.0
            else:
                TFC = np.average(crop_img(ref_img_TFC, nca)[0]) / np.average(crop_img(TFC_img, nca)[0])
            # print(TFC)
            
            for img in imgs_acq:
                corr_img = TFC_corr(img, ref_img_TFC, TFC , use_ref = use_ref)
...
def save_dict (stack_dict, dst_dir, img_name = 'transmission', start_img_numb = 0, start_acq_numb = 0, overwrite = False, **kwargs):
    ...
    for key, value in stack_dict.items():
        ...
        for acq_numb, imgs_acq in zip (tqdm(range(len(value)), desc = 'Saving Images'),value):
            ...
            for img in imgs_acq:
                ...
                file_name = os.path.join(dst_dir, key, acq_folder, key + '_' + img_name + '_' + format(idx, '05d') + '.fits')
...
def save_continuous_dict (stack_dict, dst_dir, img_name = 'transmission', start_img_numb = 0, start_acq_numb = 0, overwrite = False, **kwargs):
    ...
    for key, value in stack_dict.items():
        ...
        for acq_numb, imgs_acq in zip (tqdm(range(len(value)), desc = 'Saving Images'),value):
            ...
            acq_folder = 'Acquisition_' + format(start_acq_numb, '02d')
            idx = start_img_numb
...
def slice_values_dict (stack_dict, start_slice = 0, end_slice = '', **kwargs):
    ...
    for key, value in proc_dict.items():
        ...
        for imgs_acq in value:
            ...
            slice_set.append(slice_acq_set (imgs_acq, start_slice = start_slice, end_slice = end_slice))
...
# =============================================================================
def add_parameter_to_dict (parameters_dict, parameters): 
    new_param = dict((name,eval(name)) for name in parameters)
